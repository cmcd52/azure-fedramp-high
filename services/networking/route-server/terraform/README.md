...
```hcl
module "route_server" {
  source = "./services/networking/route-server/terraform"
...
- [ ] Verify `azurerm_virtual_hub` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Network/virtualHubs`.
