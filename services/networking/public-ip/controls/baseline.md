# Azure Public IP Address — Security Control Baseline



**Service**: Azure Public IP Address
...
This service primarily addresses control families: **SC**.
...
- **Azure Policy**: Assign the [`fedramp-high-public-ip-v1`](../policies/initiatives/fedramp-high-public-ip-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=public-ip` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Public IP Address documentation: <https://learn.microsoft.com/en-us/azure/public-ip/>
