# Azure Public IP Address — Terraform Module
...
```hcl
module "public_ip" {
  source = "./services/networking/public-ip/terraform"
...
This module implements controls in families: **SC**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_public_ip` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Network/publicIPAddresses`.
