# Azure CDN (Microsoft) — Terraform Module
...
```hcl
module "azure_cdn" {
  source = "./services/networking/azure-cdn/terraform"
...
This module implements controls in families: **SC, AU**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_cdn_frontdoor_profile` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Cdn/profiles`.
