# Azure CDN (Microsoft) — Security Control Baseline



**Service**: Azure CDN (Microsoft)
...
- **Azure Policy**: Assign the [`fedramp-high-azure-cdn-v1`](../policies/initiatives/fedramp-high-azure-cdn-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=azure-cdn` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure CDN (Microsoft) documentation: <https://learn.microsoft.com/en-us/azure/azure-cdn/>
