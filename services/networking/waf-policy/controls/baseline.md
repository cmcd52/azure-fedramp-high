# Web Application Firewall Policy — Security Control Baseline



**Service**: Web Application Firewall Policy
...
This service primarily addresses control families: **SC, SI**.
...
- **Azure Policy**: Assign the [`fedramp-high-waf-policy-v1`](../policies/initiatives/fedramp-high-waf-policy-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=waf-policy` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Web Application Firewall Policy documentation: <https://learn.microsoft.com/en-us/azure/waf-policy/>
