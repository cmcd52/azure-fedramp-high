# Web Application Firewall Policy — Terraform Module
...
```hcl
module "waf_policy" {
  source = "./services/networking/waf-policy/terraform"
...
This module implements controls in families: **SC, SI**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_web_application_firewall_policy` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Network/ApplicationGatewayWebApplicationFirewallPolicies`.
