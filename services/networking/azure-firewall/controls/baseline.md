# Azure Firewall — Security Control Baseline
...
This service primarily addresses control families: **SC, AU, SI**.
...
- **Azure Policy**: Assign the [`fedramp-high-azure-firewall-v1`](../policies/initiatives/fedramp-high-azure-firewall-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=azure-firewall` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Firewall documentation: <https://learn.microsoft.com/en-us/azure/azure-firewall/>
