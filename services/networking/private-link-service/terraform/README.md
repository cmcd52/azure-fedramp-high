...
module "private_link_service" {
  source = "./services/networking/private-link-service/terraform"
...
This module implements controls in families: **SC, AC**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_private_link_service` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Network/privateLinkServices`.
