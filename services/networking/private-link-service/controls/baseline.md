# Azure Private Link Service — Security Control Baseline



**Service**: Azure Private Link Service
...
This service primarily addresses control families: **SC, AC**.
...
- **Azure Policy**: Assign the [`fedramp-high-private-link-service-v1`](../policies/initiatives/fedramp-high-private-link-service-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=private-link-service` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Private Link Service documentation: <https://learn.microsoft.com/en-us/azure/private-link-service/>
