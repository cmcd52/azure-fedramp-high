...
module "application_gateway" {
  source = "./services/networking/application-gateway/terraform"
...
This module implements controls in families: **SC, AU, SI**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_application_gateway` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Network/applicationGateways`.
