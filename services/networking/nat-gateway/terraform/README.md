...
```hcl
module "nat_gateway" {
  source = "./services/networking/nat-gateway/terraform"
...
- [ ] Verify `azurerm_nat_gateway` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Network/natGateways`.
