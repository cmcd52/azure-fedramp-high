...
```hcl
module "vpn_gateway" {
  source = "./services/networking/vpn-gateway/terraform"
...
- [ ] Verify `azurerm_virtual_network_gateway` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Network/virtualNetworkGateways`.
