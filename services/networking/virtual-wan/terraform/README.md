...
```hcl
module "virtual_wan" {
  source = "./services/networking/virtual-wan/terraform"
...
- [ ] Verify `azurerm_virtual_wan` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Network/virtualWans`.
