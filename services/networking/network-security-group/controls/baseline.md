# Network Security Group — Security Control Baseline
...
This service primarily addresses control families: **SC, AC, AU**.
...
- **Azure Policy**: Assign the [`fedramp-high-network-security-group-v1`](../policies/initiatives/fedramp-high-network-security-group-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=network-security-group` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Network Security Group documentation: <https://learn.microsoft.com/en-us/azure/network-security-group/>
