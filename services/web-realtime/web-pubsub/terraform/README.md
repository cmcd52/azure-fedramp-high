# Azure Web PubSub — Terraform Module
...
```hcl
module "web_pubsub" {
  source = "./services/web-realtime/web-pubsub/terraform"
...
- [ ] Verify `azurerm_web_pubsub` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.SignalRService/webPubSub`.
