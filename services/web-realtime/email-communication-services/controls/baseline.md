# Azure Email Communication Services — Security Control Baseline



**Service**: Azure Email Communication Services
**ARM Resource Type**: `Microsoft.Communication/emailServices`
**Service Group**: web-realtime
...
- **Azure Policy**: Assign the [`fedramp-high-email-communication-services-v1`](../policies/initiatives/fedramp-high-email-communication-services-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=email-communication-services` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Email Communication Services documentation: <https://learn.microsoft.com/en-us/azure/email-communication-services/>
