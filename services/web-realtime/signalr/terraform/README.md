# Azure SignalR Service — Terraform Module
...
```hcl
module "signalr" {
  source = "./services/web-realtime/signalr/terraform"
...
- [ ] Verify `azurerm_signalr_service` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.SignalRService/signalR`.
