# Logic Apps Standard — Terraform Module
...
module "logic_apps_standard" {
  source = "./services/integration/logic-apps-standard/terraform"
...
- [ ] Verify `azurerm_logic_app_standard` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Web/sites`.
