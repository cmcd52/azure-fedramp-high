# Logic Apps Consumption — Terraform Module
...
```hcl
module "logic_apps_consumption" {
  source = "./services/integration/logic-apps-consumption/terraform"
...
- [ ] Verify `azurerm_logic_app_workflow` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Logic/workflows`.
