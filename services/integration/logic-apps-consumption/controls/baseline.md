# Logic Apps Consumption — Security Control Baseline



**Service**: Logic Apps Consumption
...
- **Azure Policy**: Assign the [`fedramp-high-logic-apps-consumption-v1`](../policies/initiatives/fedramp-high-logic-apps-consumption-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=logic-apps-consumption` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Logic Apps Consumption documentation: <https://learn.microsoft.com/en-us/azure/logic-apps-consumption/>
