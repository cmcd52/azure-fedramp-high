# Terraform Module: Azure Health Data Services
#
# Deploys Azure Health Data Services with FedRAMP High compliant configuration.
# NIST 800-53 Rev 5: SC-7, SC-13, SC-28, AU-12, CP-9, AC-3
...
resource "azurerm_private_endpoint" "this" {
  ...
  private_service_connection {
    name                           = "${var.name}-psc"
    private_connection_resource_id = azurerm_healthcare_workspace.this.id
    is_manual_connection           = false
    # subresource_names: verify the correct subresource for Microsoft.HealthcareApis/workspaces.
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  name                       = "${var.name}-diag"
  target_resource_id         = azurerm_healthcare_workspace.this.id
  log_analytics_workspace_id = var.log_analytics_workspace_id

  # AU-12: Enable all available log categories. Verify exact category names
  # for Microsoft.HealthcareApis/workspaces at:
