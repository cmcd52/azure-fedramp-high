# Service Bus — Security Control Baseline
...
- **In transit**: TLS 1.2 minimum with FIPS 140-2 validated cipher suites. Set `minimumTlsVersion` to `1.2`. (NIST SC-8, SC-13)
...
- **Azure Policy**: Assign the [`fedramp-high-service-bus-v1`](../policies/initiatives/fedramp-high-service-bus-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=service-bus` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Service Bus documentation: <https://learn.microsoft.com/en-us/azure/service-bus/>
