...
```hcl
module "service_bus" {
  source = "./services/integration/service-bus/terraform"
...
- [ ] Verify `azurerm_servicebus_namespace` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.ServiceBus/namespaces`.
