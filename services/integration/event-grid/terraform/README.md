...
```hcl
module "event_grid" {
  source = "./services/integration/event-grid/terraform"
...
- [ ] Verify `azurerm_eventgrid_topic` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.EventGrid/topics`.
