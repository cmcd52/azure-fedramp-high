# Azure Spring Apps — Terraform Module
...
```hcl
module "spring_apps" {
  source = "./services/compute-storage/spring-apps/terraform"
...
- [ ] Verify `azurerm_spring_cloud_service` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.AppPlatform/Spring`.
