# Azure Dedicated Host — Security Control Baseline



**Service**: Azure Dedicated Host
**ARM Resource Type**: `Microsoft.Compute/hostGroups`
...
- **Azure Policy**: Assign the [`fedramp-high-dedicated-host-v1`](../policies/initiatives/fedramp-high-dedicated-host-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=dedicated-host` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Dedicated Host documentation: <https://learn.microsoft.com/en-us/azure/dedicated-host/>
