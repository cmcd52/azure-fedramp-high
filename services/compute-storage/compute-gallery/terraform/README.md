# Azure Compute Gallery — Terraform Module
...
module "compute_gallery" {
  source = "./services/compute-storage/compute-gallery/terraform"
...
This module implements controls in families: **AC, CM**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_shared_image_gallery` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Compute/galleries`.
