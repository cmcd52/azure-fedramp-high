# Azure Compute Gallery — Security Control Baseline



**Service**: Azure Compute Gallery
**ARM Resource Type**: `Microsoft.Compute/galleries`
...
This service primarily addresses control families: **AC, CM**.
...
- **Azure Policy**: Assign the [`fedramp-high-compute-gallery-v1`](../policies/initiatives/fedramp-high-compute-gallery-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=compute-gallery` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Compute Gallery documentation: <https://learn.microsoft.com/en-us/azure/compute-gallery/>
