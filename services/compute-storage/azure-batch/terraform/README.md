...
```hcl
module "azure_batch" {
  source = "./services/compute-storage/azure-batch/terraform"
...
- [ ] Verify `azurerm_batch_account` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Batch/batchAccounts`.
