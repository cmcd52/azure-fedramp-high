# Azure Batch — Security Control Baseline
...
- **Azure Policy**: Assign the [`fedramp-high-azure-batch-v1`](../policies/initiatives/fedramp-high-azure-batch-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=azure-batch` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Batch documentation: <https://learn.microsoft.com/en-us/azure/azure-batch/>
