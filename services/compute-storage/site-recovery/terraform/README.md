# Azure Site Recovery — Terraform Module
...
module "site_recovery" {
  source = "./services/compute-storage/site-recovery/terraform"
...
This module implements controls in families: **CP, AU, SC**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_recovery_services_vault` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.RecoveryServices/vaults`.
