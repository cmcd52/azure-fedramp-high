# Azure Backup — Security Control Baseline
...
This service primarily addresses control families: **CP, AU, SC**.
...
- **Azure Policy**: Assign the [`fedramp-high-azure-backup-v1`](../policies/initiatives/fedramp-high-azure-backup-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=azure-backup` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Backup documentation: <https://learn.microsoft.com/en-us/azure/azure-backup/>
