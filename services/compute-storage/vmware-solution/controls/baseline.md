# Azure VMware Solution — Security Control Baseline



**Service**: Azure VMware Solution
**ARM Resource Type**: `Microsoft.AVS/privateClouds`
...
This service primarily addresses control families: **AC, SC, CM**.
...
- **Azure Policy**: Assign the [`fedramp-high-vmware-solution-v1`](../policies/initiatives/fedramp-high-vmware-solution-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=vmware-solution` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure VMware Solution documentation: <https://learn.microsoft.com/en-us/azure/vmware-solution/>
