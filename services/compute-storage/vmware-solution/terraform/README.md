# Azure VMware Solution — Terraform Module
...
module "vmware_solution" {
  source = "./services/compute-storage/vmware-solution/terraform"
...
This module implements controls in families: **AC, SC, CM**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_vmware_private_cloud` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.AVS/privateClouds`.
