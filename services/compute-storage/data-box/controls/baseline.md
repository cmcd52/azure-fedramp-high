# Azure Data Box — Security Control Baseline



**Service**: Azure Data Box
**ARM Resource Type**: `Microsoft.DataBox/jobs`
...
This service primarily addresses control families: **MP, SC, AU**.
...
- **Azure Policy**: Assign the [`fedramp-high-data-box-v1`](../policies/initiatives/fedramp-high-data-box-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=data-box` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Data Box documentation: <https://learn.microsoft.com/en-us/azure/data-box/>
