# Virtual Machine Scale Sets — Security Control Baseline
...
This service primarily addresses control families: **AC, SC, AU, CM, SI**.
...
- **Azure Policy**: Assign the [`fedramp-high-virtual-machine-scale-sets-v1`](../policies/initiatives/fedramp-high-virtual-machine-scale-sets-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=virtual-machine-scale-sets` tags via the Terraform module's `tags` variable (NIST CM-8).
