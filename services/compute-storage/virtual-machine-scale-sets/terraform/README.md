# Virtual Machine Scale Sets — Terraform Module
...
module "virtual_machine_scale_sets" {
  source = "./services/compute-storage/virtual-machine-scale-sets/terraform"
...
This module implements controls in families: **AC, SC, AU, CM, SI**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_linux_virtual_machine_scale_set` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Compute/virtualMachineScaleSets`.
