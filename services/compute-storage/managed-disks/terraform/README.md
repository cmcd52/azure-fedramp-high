...
module "managed_disks" {
  source = "./services/compute-storage/managed-disks/terraform"
...
- [ ] Verify `azurerm_managed_disk` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Compute/disks`.
