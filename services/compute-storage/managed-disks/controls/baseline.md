# Azure Managed Disks — Security Control Baseline



**Service**: Azure Managed Disks
**ARM Resource Type**: `Microsoft.Compute/disks`
**Service Group**: compute-storage
...
This service primarily addresses control families: **SC, CM**.
...
- **At rest**: Customer-managed key (CMK) via Azure Key Vault recommended; service-managed keys acceptable when CMK not yet supported (NIST SC-12, SC-13, SC-28).
...
- **Diagnostic settings**: Service does not natively emit diagnostic logs to Log Analytics; rely on Azure Activity Log + Defender for Cloud signals (NIST AU-2, AU-12).
...
- **Azure Policy**: Assign the [`fedramp-high-managed-disks-v1`](../policies/initiatives/fedramp-high-managed-disks-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=managed-disks` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Managed Disks documentation: <https://learn.microsoft.com/en-us/azure/managed-disks/>
