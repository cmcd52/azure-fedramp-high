# Azure Files (Premium) — Terraform Module
...
```hcl
module "azure_files_premium" {
  source = "./services/compute-storage/azure-files-premium/terraform"
...
- [ ] Verify `azurerm_storage_share` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Storage/storageAccounts/fileServices`.
