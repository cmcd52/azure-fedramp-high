# Azure NetApp Files — Terraform Module
...
```hcl
module "netapp_files" {
  source = "./services/compute-storage/netapp-files/terraform"
...
- [ ] Verify `azurerm_netapp_account` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.NetApp/netAppAccounts`.
