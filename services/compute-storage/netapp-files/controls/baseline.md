# Azure NetApp Files — Security Control Baseline
...
- **Azure Policy**: Assign the [`fedramp-high-netapp-files-v1`](../policies/initiatives/fedramp-high-netapp-files-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=netapp-files` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure NetApp Files documentation: <https://learn.microsoft.com/en-us/azure/netapp-files/>
