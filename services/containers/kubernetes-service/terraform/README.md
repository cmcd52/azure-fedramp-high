...
module "kubernetes_service" {
  source = "./services/containers/kubernetes-service/terraform"
...
This module implements controls in families: **AC, SC, AU, CM, IR**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_kubernetes_cluster` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.ContainerService/managedClusters`.
