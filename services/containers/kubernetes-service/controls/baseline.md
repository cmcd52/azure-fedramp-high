# Azure Kubernetes Service — Security Control Baseline
...
This service primarily addresses control families: **AC, SC, AU, CM, IR**.
...
- **Azure Policy**: Assign the [`fedramp-high-kubernetes-service-v1`](../policies/initiatives/fedramp-high-kubernetes-service-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=kubernetes-service` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Kubernetes Service documentation: <https://learn.microsoft.com/en-us/azure/kubernetes-service/>
