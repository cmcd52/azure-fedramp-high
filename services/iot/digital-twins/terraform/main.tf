...
#
# Deploys Azure Digital Twins with FedRAMP High compliant configuration.
...
resource "azurerm_private_endpoint" "this" {
  ...
  private_service_connection {
    ...
    # subresource_names: verify the correct subresource for Microsoft.DigitalTwins/digitalTwinsInstances.
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  name                       = "${var.name}-diag"
  target_resource_id         = azurerm_digital_twins_instance.this.id
  log_analytics_workspace_id = var.log_analytics_workspace_id

  # AU-12: Enable all available log categories. Verify exact category names
  # for Microsoft.DigitalTwins/digitalTwinsInstances at:
