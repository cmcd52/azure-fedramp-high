# Azure IoT Device Provisioning Service — Terraform Module
...
```hcl
module "iot_dps" {
  source = "./services/iot/iot-dps/terraform"
...
- [ ] Verify `azurerm_iothub_dps` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Devices/provisioningServices`.
