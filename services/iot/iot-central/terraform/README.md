# Azure IoT Central — Terraform Module
...
```hcl
module "iot_central" {
  source = "./services/iot/iot-central/terraform"
...
- [ ] Verify `azurerm_iotcentral_application` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.IoTCentral/IoTApps`.
