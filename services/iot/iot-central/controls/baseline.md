# Azure IoT Central — Security Control Baseline
...
- **Azure Policy**: Assign the [`fedramp-high-iot-central-v1`](../policies/initiatives/fedramp-high-iot-central-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=iot-central` tags via the Terraform module's `tags` variable (NIST CM-8).
