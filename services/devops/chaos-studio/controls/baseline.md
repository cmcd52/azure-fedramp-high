# Azure Chaos Studio — Security Control Baseline
...
This service primarily addresses control families: **AC, AU, CP**.
...
- **Azure Policy**: Assign the [`fedramp-high-chaos-studio-v1`](../policies/initiatives/fedramp-high-chaos-studio-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=chaos-studio` tags via the Terraform module's `tags` variable (NIST CM-8).
