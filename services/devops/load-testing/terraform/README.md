# Azure Load Testing — Terraform Module
...
```hcl
module "load_testing" {
  source = "./services/devops/load-testing/terraform"
...
- [ ] Verify `azurerm_load_test` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.LoadTestService/loadTests`.
