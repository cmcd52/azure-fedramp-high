...
#
# Deploys Azure Load Testing with FedRAMP High compliant configuration.
# NIST 800-53 Rev 5: SC-7, SC-13, SC-28, IA-2, AU-12
...
resource "azurerm_private_endpoint" "this" {
  ...
  private_service_connection {
    name                           = "${var.name}-psc"
    private_connection_resource_id = azurerm_load_test.this.id
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  name                       = "${var.name}-diag"
  target_resource_id         = azurerm_load_test.this.id
  log_analytics_workspace_id = var.log_analytics_workspace_id

  # AU-12: Enable all available log categories. Verify exact category names
  # for Microsoft.LoadTestService/loadTests at:
