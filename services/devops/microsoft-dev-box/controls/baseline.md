# Microsoft Dev Box — Security Control Baseline
...
This service primarily addresses control families: **AC, IA, AU, CM**.
...
- **Azure Policy**: Assign the [`fedramp-high-microsoft-dev-box-v1`](../policies/initiatives/fedramp-high-microsoft-dev-box-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=microsoft-dev-box` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Microsoft Dev Box documentation: <https://learn.microsoft.com/en-us/azure/microsoft-dev-box/>
