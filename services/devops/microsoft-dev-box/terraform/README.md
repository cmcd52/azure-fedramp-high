# Microsoft Dev Box — Terraform Module
...
```hcl
module "microsoft_dev_box" {
  source = "./services/devops/microsoft-dev-box/terraform"
...
This module implements controls in families: **AC, IA, AU, CM**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_dev_center` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.DevCenter/devcenters`.
