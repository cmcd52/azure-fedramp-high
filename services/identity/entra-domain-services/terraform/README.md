# Microsoft Entra Domain Services — Terraform Module
...
module "entra_domain_services" {
  source = "./services/identity/entra-domain-services/terraform"
...
This module implements controls in families: **AC, IA, AU**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_active_directory_domain_service` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.AAD/domainServices`.
