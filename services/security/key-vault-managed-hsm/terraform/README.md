# Azure Key Vault Managed HSM — Terraform Module
...
module "key_vault_managed_hsm" {
  source = "./services/security/key-vault-managed-hsm/terraform"
...
This module implements controls in families: **SC, IA, AU**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_key_vault_managed_hardware_security_module` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.KeyVault/managedHSMs`.
