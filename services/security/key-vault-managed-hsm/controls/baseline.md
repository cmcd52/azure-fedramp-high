# Azure Key Vault Managed HSM — Security Control Baseline
...
This service primarily addresses control families: **SC, IA, AU**.
...
- **Azure Policy**: Assign the [`fedramp-high-key-vault-managed-hsm-v1`](../policies/initiatives/fedramp-high-key-vault-managed-hsm-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=key-vault-managed-hsm` tags via the Terraform module's `tags` variable (NIST CM-8).
