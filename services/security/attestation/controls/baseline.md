# Azure Attestation — Security Control Baseline



**Service**: Azure Attestation
**ARM Resource Type**: `Microsoft.Attestation/attestationProviders`
...
This service primarily addresses control families: **SC, IA**.
...
- **Azure Policy**: Assign the [`fedramp-high-attestation-v1`](../policies/initiatives/fedramp-high-attestation-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=attestation` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Attestation documentation: <https://learn.microsoft.com/en-us/azure/attestation/>
