# Azure Attestation — Terraform Module
...
```hcl
module "attestation" {
  source = "./services/security/attestation/terraform"
...
This module implements controls in families: **SC, IA**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_attestation_provider` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Attestation/attestationProviders`.
