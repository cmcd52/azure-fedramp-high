# Defender External Attack Surface Management — Security Control Baseline



**Service**: Defender External Attack Surface Management
**ARM Resource Type**: `Microsoft.Easm/workspaces`
...
This service primarily addresses control families: **RA, SI**.
...
- **Azure Policy**: Assign the [`fedramp-high-defender-easm-v1`](../policies/initiatives/fedramp-high-defender-easm-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=defender-easm` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Defender External Attack Surface Management documentation: <https://learn.microsoft.com/en-us/azure/defender-easm/>
