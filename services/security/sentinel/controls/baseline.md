# Microsoft Sentinel — Security Control Baseline
...
This service primarily addresses control families: **AU, IR, SI**.
...
- **Azure Policy**: Assign the [`fedramp-high-sentinel-v1`](../policies/initiatives/fedramp-high-sentinel-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=sentinel` tags via the Terraform module's `tags` variable (NIST CM-8).
