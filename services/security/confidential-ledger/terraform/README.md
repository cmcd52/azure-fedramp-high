# Azure Confidential Ledger — Terraform Module
...
module "confidential_ledger" {
  source = "./services/security/confidential-ledger/terraform"
...
This module implements controls in families: **AU, SC, IR**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_confidential_ledger` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.ConfidentialLedger/ledgers`.
