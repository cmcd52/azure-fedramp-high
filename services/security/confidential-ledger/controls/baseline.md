# Azure Confidential Ledger — Security Control Baseline



**Service**: Azure Confidential Ledger
**ARM Resource Type**: `Microsoft.ConfidentialLedger/ledgers`
...
This service primarily addresses control families: **AU, SC, IR**.
...
- **Azure Policy**: Assign the [`fedramp-high-confidential-ledger-v1`](../policies/initiatives/fedramp-high-confidential-ledger-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=confidential-ledger` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Confidential Ledger documentation: <https://learn.microsoft.com/en-us/azure/confidential-ledger/>
