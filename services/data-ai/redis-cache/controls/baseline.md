# Azure Cache for Redis — Security Control Baseline
...
- **Azure Policy**: Assign the [`fedramp-high-redis-cache-v1`](../policies/initiatives/fedramp-high-redis-cache-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=redis-cache` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Cache for Redis documentation: <https://learn.microsoft.com/en-us/azure/redis-cache/>
