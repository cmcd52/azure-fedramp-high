...
```hcl
module "redis_cache" {
  source = "./services/data-ai/redis-cache/terraform"
...
- [ ] Verify `azurerm_redis_cache` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Cache/redis`.
