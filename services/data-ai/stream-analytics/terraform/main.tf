...
#
# Deploys Azure Stream Analytics with FedRAMP High compliant configuration.
...
resource "azurerm_private_endpoint" "this" {
  ...
  private_service_connection {
    name                           = "${var.name}-psc"
    private_connection_resource_id = azurerm_stream_analytics_job.this.id
    is_manual_connection           = false
    subresource_names              = ["streamingjobs"]
  }
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  ...
  # for Microsoft.StreamAnalytics/streamingjobs at:
