...
module "analysis_services" {
  source = "./services/data-ai/analysis-services/terraform"
...
- [ ] Verify `azurerm_analysis_services_server` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.AnalysisServices/servers`.
