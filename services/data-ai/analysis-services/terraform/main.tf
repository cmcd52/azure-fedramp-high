# Terraform Module: Azure Analysis Services
#
# Deploys Azure Analysis Services with FedRAMP High compliant configuration.
...
resource "azurerm_private_endpoint" "this" {
  ...
  private_service_connection {
    name                           = "${var.name}-psc"
    private_connection_resource_id = azurerm_analysis_services_server.this.id
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  name                       = "${var.name}-diag"
  target_resource_id         = azurerm_analysis_services_server.this.id
  log_analytics_workspace_id = var.log_analytics_workspace_id

  # AU-12: Enable all available log categories. Verify exact category names
  # for Microsoft.AnalysisServices/servers at:
