...
```hcl
module "fabric" {
  source = "./services/data-ai/fabric/terraform"
...
- [ ] Verify `azurerm_fabric_capacity` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Fabric/capacities`.
