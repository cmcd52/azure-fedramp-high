# Microsoft Fabric — Security Control Baseline



**Service**: Microsoft Fabric
**ARM Resource Type**: `Microsoft.Fabric/capacities`
...
- **Azure Policy**: Assign the [`fedramp-high-fabric-v1`](../policies/initiatives/fedramp-high-fabric-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=fabric` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Microsoft Fabric documentation: <https://learn.microsoft.com/en-us/azure/fabric/>
