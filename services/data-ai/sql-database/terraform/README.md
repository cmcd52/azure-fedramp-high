...
```hcl
module "sql_database" {
  source = "./services/data-ai/sql-database/terraform"
...
- [ ] Verify `azurerm_mssql_database` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Sql/servers/databases`.
