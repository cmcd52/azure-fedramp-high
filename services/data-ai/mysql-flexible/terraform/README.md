# Azure Database for MySQL Flexible Server — Terraform Module
...
module "mysql_flexible" {
  source = "./services/data-ai/mysql-flexible/terraform"
...
This module implements controls in families: **AC, SC, AU, IR**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_mysql_flexible_server` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.DBforMySQL/flexibleServers`.
