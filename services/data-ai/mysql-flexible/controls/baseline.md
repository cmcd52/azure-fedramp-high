# Azure Database for MySQL Flexible Server — Security Control Baseline



**Service**: Azure Database for MySQL Flexible Server
**ARM Resource Type**: `Microsoft.DBforMySQL/flexibleServers`
**Service Group**: data-ai
...
This service primarily addresses control families: **AC, SC, AU, IR**.
...
- **Azure Policy**: Assign the [`fedramp-high-mysql-flexible-v1`](../policies/initiatives/fedramp-high-mysql-flexible-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=mysql-flexible` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Database for MySQL Flexible Server documentation: <https://learn.microsoft.com/en-us/azure/mysql-flexible/>
