...
```hcl
module "synapse" {
  source = "./services/data-ai/synapse/terraform"
...
- [ ] Verify `azurerm_synapse_workspace` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Synapse/workspaces`.
