# Azure AI Services (Umbrella) — Terraform Module
...
```hcl
module "ai_services_umbrella" {
  source = "./services/data-ai/ai-services-umbrella/terraform"
...
- [ ] Verify `azurerm_cognitive_account` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.CognitiveServices/accounts`.
