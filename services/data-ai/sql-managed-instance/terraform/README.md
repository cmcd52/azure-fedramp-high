...
module "sql_managed_instance" {
  source = "./services/data-ai/sql-managed-instance/terraform"
...
- [ ] Verify `azurerm_mssql_managed_instance` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Sql/managedInstances`.
