...
```hcl
module "databricks" {
  source = "./services/data-ai/databricks/terraform"
...
- [ ] Verify `azurerm_databricks_workspace` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Databricks/workspaces`.
