# Azure Database for PostgreSQL Flexible Server — Security Control Baseline
...
- **Azure Policy**: Assign the [`fedramp-high-postgresql-flexible-v1`](../policies/initiatives/fedramp-high-postgresql-flexible-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=postgresql-flexible` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Database for PostgreSQL Flexible Server documentation: <https://learn.microsoft.com/en-us/azure/postgresql-flexible/>
