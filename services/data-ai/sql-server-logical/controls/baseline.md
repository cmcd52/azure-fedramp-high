# Azure SQL Server (Logical) — Security Control Baseline



**Service**: Azure SQL Server (Logical)
...
This service primarily addresses control families: **AC, SC, AU, IA**.
...
- **Azure Policy**: Assign the [`fedramp-high-sql-server-logical-v1`](../policies/initiatives/fedramp-high-sql-server-logical-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=sql-server-logical` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure SQL Server (Logical) documentation: <https://learn.microsoft.com/en-us/azure/sql-server-logical/>
