# Azure SQL Server (Logical) — Terraform Module
...
```hcl
module "sql_server_logical" {
  source = "./services/data-ai/sql-server-logical/terraform"
...
This module implements controls in families: **AC, SC, AU, IA**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_mssql_server` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Sql/servers`.
