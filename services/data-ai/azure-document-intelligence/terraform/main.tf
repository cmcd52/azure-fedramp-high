# Terraform Module: Azure Document Intelligence
#
# Deploys an Azure Document Intelligence (Cognitive Services, kind:
# FormRecognizer) account with FedRAMP High compliant defaults: no
# public network access, Private Endpoint only, default-deny network
# rules, system-assigned managed identity, CMK for data at rest,
...
#
# NOTE: Azure Document Intelligence uses the
# Microsoft.CognitiveServices/accounts resource type with
# kind = "FormRecognizer".
...
resource "azurerm_private_endpoint" "this" {
  ...
  name                = "${var.docintel_account_name}-pe"
  location            = var.location
  resource_group_name = var.resource_group_name
  subnet_id           = var.subnet_id

  private_service_connection {
    name                           = "${var.docintel_account_name}-psc"
...
# OMB M-21-31: EL2 for audit and request/response logs
resource "azurerm_monitor_diagnostic_setting" "this" {
  name                       = "${var.docintel_account_name}-diag"
  ...
  # AU-12: Request/Response log — document processing API call metadata
  enabled_log {
