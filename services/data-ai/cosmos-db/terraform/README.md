...
```hcl
module "cosmos_db" {
  source = "./services/data-ai/cosmos-db/terraform"
...
This module implements controls in families: **AC, SC, AU, IR, CP**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_cosmosdb_account` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.DocumentDB/databaseAccounts`.
