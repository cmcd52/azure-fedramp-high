# Azure Stack Edge — Security Control Baseline
...
This service primarily addresses control families: **SC, MP, CM**.
...
- **Azure Policy**: Assign the [`fedramp-high-stack-edge-v1`](../policies/initiatives/fedramp-high-stack-edge-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=stack-edge` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Stack Edge documentation: <https://learn.microsoft.com/en-us/azure/stack-edge/>
