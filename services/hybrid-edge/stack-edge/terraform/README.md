# Azure Stack Edge — Terraform Module
...
```hcl
module "stack_edge" {
  source = "./services/hybrid-edge/stack-edge/terraform"
...
This module implements controls in families: **SC, MP, CM**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_databox_edge_device` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.DataBoxEdge/dataBoxEdgeDevices`.
