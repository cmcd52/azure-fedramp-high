# Azure Local (Azure Stack HCI) — Terraform Module
...
```hcl
module "azure_local" {
  source = "./services/hybrid-edge/azure-local/terraform"
...
This module implements controls in families: **CM, SC, AC**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_stack_hci_cluster` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.AzureStackHCI/clusters`.
