# Azure Local (Azure Stack HCI) — Security Control Baseline



**Service**: Azure Local (Azure Stack HCI)
**ARM Resource Type**: `Microsoft.AzureStackHCI/clusters`
...
This service primarily addresses control families: **CM, SC, AC**.
...
- **Azure Policy**: Assign the [`fedramp-high-azure-local-v1`](../policies/initiatives/fedramp-high-azure-local-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=azure-local` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Local (Azure Stack HCI) documentation: <https://learn.microsoft.com/en-us/azure/azure-local/>
