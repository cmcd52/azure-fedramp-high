...
```hcl
module "automation" {
  source = "./services/management/automation/terraform"
...
This module implements controls in families: **CM, AC, AU**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_automation_account` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Automation/automationAccounts`.
