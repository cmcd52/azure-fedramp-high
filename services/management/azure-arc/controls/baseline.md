# Azure Arc — Security Control Baseline
...
This service primarily addresses control families: **AC, CM, AU**.
...
- **Azure Policy**: Assign the [`fedramp-high-azure-arc-v1`](../policies/initiatives/fedramp-high-azure-arc-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=azure-arc` tags via the Terraform module's `tags` variable (NIST CM-8).
