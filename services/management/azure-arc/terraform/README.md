...
```hcl
module "azure_arc" {
  source = "./services/management/azure-arc/terraform"
...
This module implements controls in families: **AC, CM, AU**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_arc_machine` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.HybridCompute/machines`.
