# Azure Policy — Security Control Baseline



**Service**: Azure Policy
**ARM Resource Type**: `Microsoft.Authorization/policyDefinitions`
...
This service primarily addresses control families: **CM, AC, RA**.
...
- **Azure Policy**: Assign the [`fedramp-high-azure-policy-v1`](../policies/initiatives/fedramp-high-azure-policy-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=azure-policy` tags via the Terraform module's `tags` variable (NIST CM-8).
