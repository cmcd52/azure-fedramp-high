...
```hcl
module "azure_policy" {
  source = "./services/management/azure-policy/terraform"
...
This module implements controls in families: **CM, AC, RA**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_policy_definition` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Authorization/policyDefinitions`.
