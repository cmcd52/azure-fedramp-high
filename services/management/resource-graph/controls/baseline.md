# Azure Resource Graph — Security Control Baseline



**Service**: Azure Resource Graph
**ARM Resource Type**: `Microsoft.ResourceGraph/queries`
...
This service primarily addresses control families: **AU, CM**.
...
- **Azure Policy**: Assign the [`fedramp-high-resource-graph-v1`](../policies/initiatives/fedramp-high-resource-graph-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=resource-graph` tags via the Terraform module's `tags` variable (NIST CM-8).
...
- Azure Resource Graph documentation: <https://learn.microsoft.com/en-us/azure/resource-graph/>
