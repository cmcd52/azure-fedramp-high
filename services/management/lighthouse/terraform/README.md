...
```hcl
module "lighthouse" {
  source = "./services/management/lighthouse/terraform"
...
- [ ] Verify `azurerm_lighthouse_definition` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.ManagedServices/registrationDefinitions`.
