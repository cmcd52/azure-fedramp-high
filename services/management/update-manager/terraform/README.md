# Azure Update Manager — Terraform Module
...
module "update_manager" {
  source = "./services/management/update-manager/terraform"
...
This module implements controls in families: **SI, CM**.
See [../controls/baseline.md](../controls/baseline.md) for control-by-control evidence.

## Pending Review Items

- [ ] Verify `azurerm_maintenance_configuration` argument schema against current azurerm provider version.
...
- [X] ~~Replace `subresource_names = ["TODO_SUBRESOURCE_NAME"]`~~ in the Private Endpoint with the correct subresource name for `Microsoft.Maintenance/maintenanceConfigurations`.
