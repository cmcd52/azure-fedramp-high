# Terraform Module: Azure Database Migration Service
#
# Deploys Azure Database Migration Service with FedRAMP High compliant configuration.
...
resource "azurerm_private_endpoint" "this" {
  ...
  private_service_connection {
    name                           = "${var.name}-psc"
    private_connection_resource_id = azurerm_database_migration_service.this.id
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  name                       = "${var.name}-diag"
  target_resource_id         = azurerm_database_migration_service.this.id
  log_analytics_workspace_id = var.log_analytics_workspace_id

  # AU-12: Enable all available log categories. Verify exact category names
  # for Microsoft.DataMigration/services at:
