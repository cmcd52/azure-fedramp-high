# Azure Migrate — Security Control Baseline



**Service**: Azure Migrate
**ARM Resource Type**: `Microsoft.Migrate/projects`
...
This service primarily addresses control families: **AC, AU, CM**.
...
- **Azure Policy**: Assign the [`fedramp-high-azure-migrate-v1`](../policies/initiatives/fedramp-high-azure-migrate-v1.json) initiative at the management-group or subscription scope (NIST CM-2, CM-6).
- **Resource locks**: Apply `CanNotDelete` to production resources (NIST CM-5).
- **Tagging**: Apply `compliance-framework=FedRAMP-High` and `service=azure-migrate` tags via the Terraform module's `tags` variable (NIST CM-8).
